...
;;;;;;;;;;;SETTING EACH CELL DATA LOOP;;;;;;;;;;;;;;;;;;;
 ...
 loop1:
     ...
     CELL_IN_TRIANGLE:
        ...
        jz FIRST_IN_ROW 
        cmp DL,DH  ; IF COL==0 GO TO  first_element_in row
        jz LAST_IN_ROW
        
        ; IF COL >0 && COLS<CUR_ROW    ;BASE +(ROW SIZE*ROW_OFFSE+ITEM OFFSET)*CELL SIZE
        MOV BX,0000H
        MOV DX,0000H 
        MOV AX,0000H
        ADD BL,[ROWS_B]
        ...
        SUB BX,[ROWS_W]
        SUB BX,[ROWS_W]           ;; INDEX OF ELEMENT ARR[i-1][j] 
        MOV DX,arr1+[BX]         ;;DATA=ARR[i-1]
        DEC BX
        DEC BX                 ;;INDEX OF ELEMENT ARR[i-1][j-1]
        ...
        FIRST_IN_ROW:
            ...
            MOV DX,arr1[0]    
            MOV BX,[CELL_OFFSET]
            MOV [arr1+BX],DX    ; BASE +ROW SIZE*ROW_OFFSET+ROW_OFFSET *2
            MOV BX,0000H
            MOV DX,0000H
            MOV AX,0000H
        LAST_IN_ROW:
